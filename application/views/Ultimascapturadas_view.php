...
<script type="text/javascript"> //pega os dados do banco e coloca na tabela a cada 3s
    var j = 0, cont = 0, checkClicados = 0, lock = 0, limitGraf = 4, contGraf = 1, atualizando = 0, dadosAtual, ultimoCodCaptura = 0;
    ...
    function atualizaTable() {
        $.ajax({
            ...
            success: function (dados) {
                if (dados) {
//                    alert(ultimoCodCaptura + "  " + dados[j].codCaptura);
                    
                    while (ultimoCodCaptura !== dados[j].codCaptura) {
                        insereLinha(dados[j]);
                        if (lock === 1){
                        ...
                        ultimoCodCaptura = dados[j].codCaptura;
                        j++;
                    }
                    if(lock === 0) {
    ...
    function geraGraficoInit(dados){
        for(var i = limitGraf; i > 0; i--){
            ...
            geraGraficoPrimeiraLinha(dados[j-i].codCaptura, 0);
        }
    }
    ...
    function iniciaFFT() {
        for(var i = dadosAtual.length; i >= 1; i--){
            var cod = dadosAtual[j-i];
            if(document.getElementById("container"+cod))
                insereFFT(cod);
        }
    }
...
//            var checkbox = $('#checkboxequip').find('tr'); 
//            tbody.each(function(i,e){
//                $(e).prop("style", "display:block");
//            });
            for(var i = j; i > 0; i--){
                $('.w'+i).prop("disabled",true);
            }
                
//            $('#checkboxequip').prop("disabled",true);
            window.setTimeout(atualizaTable, 1000);
        } else {
            for(var i = j; i > 0; i--){
                $('.w'+i).prop("disabled",false);
            }
            atualizando = 1;
        }
    }
