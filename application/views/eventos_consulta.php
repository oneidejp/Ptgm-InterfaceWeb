							...
							<table id="myTable" class="table table-striped table-bordered sortable">
								...
								<tbody>
									...
									foreach ($eventos as $dados) {
										...
										<tr>
											<td>
												<a href="<?= base_url('')?>index.php/eventos/apagar_eventos/<?= $dados->codEvento; ?>" onClick="return confirm('<?= $this->lang->line('msg_confirm_delete')." ".$this->lang->line('kind_of_event')." ".$this->lang->line('code').": ".$dados->codEvento; ?> ?')">
													<img src="<?= base_url('includes/imagens/delete.png') ?>"></a>
													<a href="<?= base_url('')?>index.php/eventos/editar_eventos/<?= $dados->codEvento; ?>">
														<img src="<?= base_url('includes/imagens/edit.png') ?>"></a>
													</td>
													<td><a href="<?= base_url('')?>index.php/eventos/editar_eventos/<?= $dados->codEvento; ?>"><?= $dados->codEvento; ?></a></td>
													<td><a href="<?= base_url('')?>index.php/eventos/editar_eventos/<?= $dados->codEvento; ?>"><?= $dados->desc; ?></a></td>
													<td><a href="<?= base_url('')?>index.php/eventos/editar_eventos/<?= $dados->codEvento; ?>"><?= $dados->formaOnda?>  - <?php if($dados->formaOnda==0){echo "Não";}else{echo "Sim";} ?></a></td>
												</tr>
												<?php		};
												?>
											</tbody>
