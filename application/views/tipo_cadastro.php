		...
		<div class="row-fluid">
			...
			<div class="span10" id="centro">
				...
				<ul class="abas">
					...
					<li id="cadastro" style="background-color: #A9A9A9;"><a href="<?= base_url('index.php/tipo?link=cadastro') ?>" ><?= $this->lang->line('cadastre'); ?></a></li>
				</ul>
				<div id="aba">
					<div class="row-fluid">
						...
						<?php 
						if ($tipo=='cadastro') { ?>
							<form class="form-signin" role="form" method="post" action="<?= base_url('index.php/tipo/create_tipo') ?>">
								<fieldset>
									<legend><?= $this->lang->line('page_title_cadastre_kind'); ?></legend>
									<h4 class="form-signin-heading"><?= $this->lang->line('code').":"; ?></h4>
									<input type="text" class="form-control" placeholder="<?= $this->lang->line('code'); ?>" readonly="readonly" autofocus name="codTipo">
							...
							<form class="form-signin" role="form" method="post" action="<?= base_url('index.php/tipo/editar_tipo/'.$tipo->codTipo) ?>">
								<fieldset>
									<legend><?= $this->lang->line('form_alter_title_kind'); ?></legend>
									<h4 class="form-signin-heading"><?= $this->lang->line('code').":"; ?></h4>
									<input type="text" class="form-control" readonly="readonly" autofocus name="codTipo"  value="<?= $tipo->codTipo; ?>">
									<h4 class="form-signin-heading"><?= $this->lang->line('description').":"; ?></h4>
									<input type="text" class="form-control" placeholder="<?= $this->lang->line('description'); ?>" required autofocus name="desc" value="<?= $tipo->desc; ?>"><br/>
									<button class="btn btn-lg btn-primary" type="submit"><?= $this->lang->line('alter'); ?></button>
									<button class="btn btn-lg btn-primary" type="submit" formaction="<?= base_url('index.php/tipo'); ?>"><?= $this->lang->line('cancel'); ?></button>
								</fieldset>
							</form>	
							<?php
							if($this->session->flashdata('msg'))?>
